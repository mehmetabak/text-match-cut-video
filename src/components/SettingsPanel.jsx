    ...
    return (
        <div className="bg-zinc-900/95 backdrop-blur-sm border border-zinc-700/50 p-4 md:p-6 rounded-lg h-full shadow-lg flex flex-col">
            ...
            <div className="flex-grow space-y-5 border-t border-zinc-700 mt-4 pt-4 overflow-y-auto pr-2">
                ...
                <SegmentedControl label={t('videoLengthLabel', lang)} options={['Short', 'Medium', 'Long']} value={settings.videoLength} onChange={(v) => setSetting('videoLength', v)} />
                <SegmentedControl label={t('blurLabel', lang)} options={['Low', 'Medium', 'High']} value={settings.blurIntensity} onChange={(v) => setSetting('blurIntensity', v)} />
                <div className="pt-2">
                    <label className="block text-sm font-medium text-gray-400">{t('speedLabel', lang)}: {settings.speed}x</label>
                    <input type="range" name="speed" min="0.5" max="2.5" step="0.25" value={settings.speed} onChange={handleInputChange} className="w-full h-2 bg-zinc-700 rounded-lg appearance-none cursor-pointer accent-indigo-500" />
                </div>
            ...
            <div className="border-t border-zinc-700 pt-4 flex-shrink-0">
                ...
                <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => setSetting('format', 'horizontal')} className={`p-2 rounded-md transition-colors font-semibold ${settings.format === 'horizontal' ? 'bg-accent text-white ring-2 ring-offset-2 ring-offset-zinc-900 ring-accent' : 'bg-zinc-700 hover:bg-zinc-600'}`}>{t('formatHorizontal', lang)}</button>
                    <button onClick={() => setSetting('format', 'vertical')} className={`p-2 rounded-md transition-colors font-semibold ${settings.format === 'vertical' ? 'bg-accent text-white ring-2 ring-offset-2 ring-offset-zinc-900 ring-accent' : 'bg-zinc-700 hover:bg-zinc-600'}`}>{t('formatVertical', lang)}</button>
                </div>
            </div>
